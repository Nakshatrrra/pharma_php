...
if ( !$connection ) {
    ...
    // adding medicine and all medicine addition to code
    elseif ( 'addMedicine' == $action ) {
        $ref_id = $_REQUEST['ref_id'] ?? '';
        $name = $_REQUEST['name'] ?? '';
        $price = $_REQUEST['price'] ?? '';
        
        if ( $ref_id && $name && $price ) {
            $hashPassword = password_hash( $password, PASSWORD_BCRYPT );
            $query = "INSERT INTO medicines(ref_id,name,price) VALUES ('{$ref_id}','$name','$price')";
            mysqli_query( $connection, $query );
            header( "location:index.php?id=allMedicine" );
        }
    }
    
    elseif ( 'updateMedicine' == $action ) {
        $ref_id = $_REQUEST['ref_id'] ?? '';
        $name = $_REQUEST['name'] ?? '';
        $price = $_REQUEST['price'] ?? '';

        if ( $fname && $lname && $lname && $phone ) {
            $query = "UPDATE salesmans SET ref_id='{$ref_id}', name='{$name}', price='$price' WHERE id='{$id}'";
            mysqli_query( $connection, $query );
            header( "location:index.php?id=allMedicine" );
        }
    }
